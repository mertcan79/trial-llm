...
def evaluate_confidence(extracted_data, question):
    ...
    for field, weight in required_fields.items():
        if any(field in feature['description'].lower() for feature in extracted_data.get('extracted_features', [])):
            score += weight

    # Check if the question is directly related to the field (higher importance)
    if 'outcomes' in question.lower() and 'outcomes' in extracted_data.get('description', '').lower():
